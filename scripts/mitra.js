const MITRA_RUNTIME = {
    dataPromise: null,
    galleryRenderedFor: null
};

window.loadMitraReplica = async function () {
        try {
          if (!MITRA_RUNTIME.dataPromise) {
            MITRA_RUNTIME.dataPromise = Promise.all([
              fetch('data/global.json').then(response => {
                if (!response.ok) {
                  throw new Error(`global.json HTTP ${response.status}`);
                }
                return response.json();
              }),
              fetch('data/mitra.json').then(response => {
                if (!response.ok) {
                  throw new Error(`mitra.json HTTP ${response.status}`);
                }
                return response.json();
              })
            ]);
          ...
          const [globalData, dataMitra] = await MITRA_RUNTIME.dataPromise;
          ...
          function handleMitraRouting() {
            ...
            const resolvedKey = dataMitra[mitraKey] ? mitraKey : 'default';
            ...
            if (currentMitra) {
              ...
              // SOCIAL MEDIA
              const socialLinks = {
                  '.mitra-fb': currentMitra.facebook,
                  '.mitra-tiktok': currentMitra.tiktok,
                  '.mitra-telegram': currentMitra.telegram,
                  '.mitra-ig': currentMitra.instagram,
                  '.mitra-yt': currentMitra.youtube
              };
              ...
              if (!MITRA_RUNTIME.galleryRendered) {
                MITRA_RUNTIME.galleryRendered = true;

                  if (MITRA_RUNTIME.galleryRenderedFor !== resolvedKey) {
                    MITRA_RUNTIME.galleryRenderedFor = resolvedKey;

                    const renderGallery = () => renderPhotoGallery(activePhotos);

                    if ('requestIdleCallback' in window) {
                        requestIdleCallback(renderGallery, { timeout: 3000 });
                    } else {
  ...
  bannerWrapper &&
  banners.length > 0 &&
  !bannerWrapper.dataset.initialized
) {
  bannerWrapper.dataset.initialized = 'true';
            ...
            // Jangan melakukan pergantian slide ketika tab/browser tidak terlihat.
...
window.addEventListener(
  ...
  { passive: true }
          ...
          if ('requestIdleCallback' in window) {
            requestIdleCallback(renderPaket, { timeout: 3000 });
          } else {
            setTimeout(renderPaket, 1500);
          }

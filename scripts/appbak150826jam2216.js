...
function showDocTab(index, btnEl) {
  ...
  if (index === 2 && DOM.testimonialContainer?.childElementCount === 0) {
    renderTestimonials();
  }
...
if ('requestIdleCallback' in window) {
  requestIdleCallback(renderSecondaryContent, { timeout: 1500 });
} else {
  setTimeout(renderSecondaryContent, 1000);
}
});
